#!/bin/bash
original_dir="/data/homeassistant/config/custom_components/midea_ac_lan"
...
if [ -d "$original_dir" ]; then
    ...
    # 定义新文件夹名称
    new_dir="/data/homeassistant/config/custom_components/midea_ac_lan_old_${current_time}"
...
# 配置参数
TARGET_DIR="/data/homeassistant/config/custom_components/midea_ac_lan"
ZIP_FILE="/tmp/midea_ac_lan_latest.zip"
GITHUB_REPO="wuwentao/midea_ac_lan"
...
# 获取最新 release 下载链接
echo "获取最新版 midea_ac_lan.zip 下载链接..."
ZIP_URL=$(curl -sL "https://api.github.com/repos/$GITHUB_REPO/releases/latest" | \
          jq -r '.assets[] | select(.name == "midea_ac_lan.zip") | .browser_download_url')
